...
export function CanvasAppsList({ canvasApps }: CanvasAppsListProps) {
  ...
  if (canvasApps.length === 0) {
    return (
      <EmptyState
        type="generic"
        title="No Canvas Apps Found"
        message="No canvas apps were found in the selected solution(s)."
      />
    );
  }
  ...
  return (
    <div className={shared.container} style={{ marginTop: tokens.spacingVerticalL }}>
      <FilterBar
        ...
        searchPlaceholder="Search canvas apps..."
        filteredCount={searched.length}
        totalCount={sorted.length}
        itemLabel="canvas apps"
      />
