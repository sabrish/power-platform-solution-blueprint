    ...
    try {
      ...
      const result = await this.client.queryMetadata<DetailedEntityMetadata>(
        'EntityDefinitions',
        {
          ...
          expand: 'Attributes($select=LogicalName,SchemaName,DisplayName,AttributeType,IsPrimaryId,IsPrimaryName,IsValidForCreate,IsValidForUpdate,IsValidForRead,RequiredLevel,Description),Keys($select=LogicalName,DisplayName,KeyAttributes)',
        }
      );
